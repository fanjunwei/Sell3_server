...
def getParam(request):
    ...
    ap = {'name': name.encode('utf-8'), 'number': number, 'phone': tel, 'address': address.encode('utf-8'),
...
def autoSaveTel(request):
    ...
    return render_to_response('oa/truename.html', RequestContext(request,
                                                                 {'count': Truename.objects.filter(status=0).count(),
                                                                  'failcount': Truename.objects.filter(
                                                                      status=2).count()}))
